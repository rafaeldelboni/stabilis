...
const Frontmatter = struct { frontmatter: []const u8, body: []const u8 };
...
pub fn parse(source: []const u8) Frontmatter {
    const delimiter = "---";
    const open_delimiter = delimiter ++ "\n";
    const close_delimiter = "\n" ++ delimiter;
    if (!startsWithFrontmatter(delimiter, source))
        return .{ .frontmatter = "", .body = source };
    if (sliceBetween(source, open_delimiter, close_delimiter, 0)) |frontmatter| {
    ...
    } else return .{ .frontmatter = "", .body = source };
}
...
test "parse returns full source as body when no frontmatter" {
    const source = "# Just a page\nNo frontmatter here.";
    const result = parse(source);
    try std.testing.expectEqualSlices(u8, "", result.frontmatter);
    try std.testing.expectEqualSlices(u8, source, result.body);
}

test "parse returns full source as body when no closing delimiter" {
    const source =
        \\---
        \\title: Orphan
    ;
...
test "parse handles empty frontmatter" {
    ...
    const result = parse(source);
    try std.testing.expectEqualSlices(u8, "", result.frontmatter);
    try std.testing.expectEqualSlices(u8, "# Hello", result.body);
}
...
test "parse handles frontmatter with no body" {
    ...
    try std.testing.expectEqualSlices(u8, "", result.body);
}
